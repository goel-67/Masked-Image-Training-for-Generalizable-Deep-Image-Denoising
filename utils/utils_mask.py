...
def add_Gaussian_noise_color(img, noise_level1, noise_level2):
    img = img.astype(np.float32)  # Convert img to float32
    noise = np.random.normal(0, noise_level1 / 255.0, img.shape).astype(np.float32)
    img += noise
    img = np.clip(img, 0, 1)  # Ensure the pixel values are within the valid range
    return img
