...
def add_impulse_noise(image, prob=0.01):
    """
    Adds impulse (salt-and-pepper) noise to a PyTorch tensor image.

    Parameters:
    - image: Input image as a PyTorch Tensor (C x H x W).
    - prob: Probability of altering each pixel.
    ...
    random_matrix = torch.rand_like(noisy_image)
...
def add_quantization_noise(image, bit_depth=4):
    """
    Adds quantization noise by reducing the bit depth of the image.
    ...
    Returns:
    - quantized_image: Image with reduced bit depth.
    """
    max_val = 2 ** bit_depth - 1
    quantized_image = torch.floor(image * max_val) / max_val
    return quantized_image


def add_thermal_noise(image, mean=0.0, var=10.0):
    """
    Adds thermal (Gaussian) noise to the image.
    ...
    """
    sigma = var ** 0.5
    gaussian_noise = torch.normal(mean=mean, std=sigma / 255.0, size=image.shape, device=image.device)
...
def add_poisson_noise(image):
    ...
    """
    # Scale image to [0, 255] for Poisson distribution
    vals = image * 255.0
    # Apply Poisson noise
    noisy_image = torch.poisson(vals)
    # Scale back to [0, 1]
    noisy_image = noisy_image / 255.0
    return torch.clamp(noisy_image, 0.0, 1.0)

# Function to apply combined noise
def add_combined_noise(image, impulse_prob=0.01, quantization_bit_depth=4, thermal_var=10):
    """
    Applies a combination of impulse, quantization, thermal, and Poisson noise to the image.

    Parameters:
    - image: Input image as a PyTorch Tensor (C x H x W).
    - impulse_prob: Probability for impulse noise.
    - quantization_bit_depth: Bit depth for quantization noise.
    - thermal_var: Variance for thermal noise.

    Returns:
    - noisy_image: Image with combined noise added.
    """
    image = add_impulse_noise(image, prob=impulse_prob)
    image = add_quantization_noise(image, bit_depth=quantization_bit_depth)
    image = add_thermal_noise(image, var=thermal_var)
    image = add_poisson_noise(image)
    return image'''

# adding distinct new noises
import numpy as np
...
def add_quantization_noise(image, levels=16):
    ...
    quantized_image = np.round(image / (255 / (levels - 1))) * (255 / (levels - 1))
    return quantized_image

def add_thermal_noise(image, mean=0, std=10):
    """Add thermal noise (Gaussian) to the image."""
