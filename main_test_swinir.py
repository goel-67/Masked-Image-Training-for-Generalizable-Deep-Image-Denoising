...
# Helper function to remove 'module.' prefix
...
def main():
    ...
    parser.add_argument('--name', type=str, default="test", help='Name for the test run.')
    args = parser.parse_args()

    # Parse options from JSON file
...
def define_model(args):
    ...
    # Add other task definitions if necessary
    ...
    if any(k.startswith('module.') for k in state_dict.keys()):
        ...
        state_dict = remove_module_prefix(state_dict, 'module.')
...
def get_image_pair(args, path):
    ...
    if args.task in ['masked_denoising', 'classical_sr', 'lightweight_sr']:
        ...
        # Attempt to read PNG first, fallback to TIF
        try:
            imgext = '.png'
            img_lq = cv2.imread(f'{args.folder_lq}/{imgname}{imgext}', cv2.IMREAD_COLOR).astype(np.float32) / 255.
            if img_lq is None:
            ...
            img_lq = cv2.imread(f'{args.folder_lq}/{imgname}{imgext}', cv2.IMREAD_COLOR).astype(np.float32) / 255.
            if img_lq is None:
                raise FileNotFoundError(f"Low-quality image not found for {imgname}")
    ...
    # 006 JPEG compression artifact reduction (load gt image and generate lq image on-the-fly)
        ...
        img_gt = cv2.imread(path, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.
        result, encimg = cv2.imencode('.jpg', img_gt * 255., [int(cv2.IMWRITE_JPEG_QUALITY), args.jpeg])
        img_lq = cv2.imdecode(encimg, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.
        img_gt = np.expand_dims(img_gt, axis=2)
        img_lq = np.expand_dims(img_lq, axis=2)
